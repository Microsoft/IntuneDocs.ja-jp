...
> [!NOTE]
> Azure Active Directory (Azure AD) からユーザーを削除する前に、そのユーザーに関連付けられているすべてのデバイスに対して、**[出荷時の設定にリセット]** または **[会社データを削除する]** アクションを使用します。 Azure AD から管理対象デバイスを持っているユーザーを削除すると、Intune でこれらのデバイスに対して、出荷時の設定にリセットまたは会社データの削除を実行できなくなります。

## <a name="factory-reset"></a>工場出荷時のリセット

**[出荷時の設定にリセット]** アクションでは、デバイスを出荷時の既定の設定に復元します。 出荷時の設定にリセットでは、すべての会社とユーザーのデータと設定が復元されます。 デバイスは Intune の管理から削除されます。 出荷時の設定にリセットは、デバイスを新しいユーザーに譲渡する前や、デバイスの紛失または盗難時に、デバイスをリセットする場合に役立ちます。 **[出荷時の設定にリセット]** の選択は慎重に行ってください。 デバイス上のデータが復元できなくなります。
