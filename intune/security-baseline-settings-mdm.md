...
ms.locfileid: "67418947"
---
# <a name="mdm-security-baseline-settings-for-intune"></a>Intune 用の MDM セキュリティ ベースラインの設定  

Windows 10 を実行するデバイス用の Microsoft Intune でサポートされている、またはそれ以降である MDM セキュリティ ベースラインの設定を表示します。 この基準での設定の既定値は、該当するデバイスは、推奨される構成を表し、他のセキュリティ ベースラインから既定値を基準に一致しないがあります。  

最新の基準バージョンは**Spring 2019 更新 (19 H 1) 用のセキュリティ ベースラインの MDM**  

この基準の最新バージョンで以前のバージョンからの変更点については、次を参照してください。[新しいテンプレートの変更点](#whats-changed-in-the-new-template)します。  

> [!NOTE]  
> リリースによって置き換えられたプレビュー MDM のセキュリティ基準年 6 月の 2019年で、 *Spring 2019 更新 (19 H 1) 用のセキュリティ ベースラインの MDM* generaly がテンプレート (プレビュー) ではなく使用できます。 プロファイルの可用性の前に作成された、 *Spring 2019 更新 (19 H 1) 用のセキュリティ ベースラインの MDM*ベースラインを反映するように更新されない、設定と Spring 2019 Update (の MDM セキュリティ基準に含まれる値19 H 1) のバージョン。  プレビュー テンプレートに基づいて新しいプロファイルを作成することはできませんは、編集し、プレビュー テンプレートに基づいて以前に作成したプロファイルを使用して続行します。   
  
Intune でのセキュリティ ベースラインを使用する方法については、次を参照してください。[セキュリティ基準を使用して](security-baselines.md)します。  
...
- **Kernel DMA Protection と互換性のない外部デバイスの列挙**  
  このポリシーの目的は、外部の DMA 対応デバイスに対して追加のセキュリティを提供します。 これにより、DMA の再マッピング/デバイス メモリの分離およびサンドボックス化と互換性のない外部 DMA 対応デバイスの列挙をより詳細に制御できます。 このポリシーが有効になるのは、Kernel DMA Protection がシステム ファームウェアによってサポートされていて、それによって有効にされた場合のみです。 カーネルの DMA 保護は、ポリシーを使用して、またはエンドユーザーが制御できないプラットフォームの機能です。 これは、製造時にシステムによってサポートされている必要があります。 システムがカーネル DMA 保護をサポートしているかを確認するには、MSINFO32.exe の概要 ページで、カーネルの DMA 保護フィールドを確認してください。  
...
- **Internet Explorer の暗号化のサポート**  
  ...
  **既定の**: 2 つの項目: TLS 1.1 および TLS v1.2  
  *この設定の選択可能なオプションを表示する下矢印を選択します。*
...
- **要請されたリモート アシスタンス**  
  ...
  このポリシー設定を有効にすると、ヘルパーによるリモート アシスタンスの提供に、次の 2 つの方法が使用できます。[ヘルパーにコンピューターの閲覧のみを許可する] と [ヘルパーにコンピューターのリモート制御を許可する] です。 "最大チケット time"ポリシー設定の上限を設定の時間を電子メールやファイル転送を使用して作成されたリモート アシスタンスの招待は開いたままことができます。 [電子メールの招待の送信方法を選択する] 設定によって、リモート アシスタンスの招待の送信に使用する電子メール規格を指定できます。 お使いの電子メールのプログラムによって、Mailto 規格 (招待を受信する側がインターネット リンク経由で接続) か、または SMAPI (簡易 MAPI) 規格 (招待を電子メールのメッセージに添付) を使用できます。 Windows Vista では SMAPI だけがサポートされている方法であるため、このポリシー設定は使用できません。 このポリシー設定を有効にした場合は、リモート アシスタンスが通信できるように、適切なファイアウォールの例外を有効にする必要もあります。  
  [詳細情報](https://go.microsoft.com/fwlink/?linkid=2067198)

  **既定の**: リモート アシスタンスを無効にします。

  設定すると*リモート アシスタンスを有効にする*、次の追加設定を構成します。  
  - **リモート アシスタンスのアクセス許可を要請します。**  
    **既定値**: ビュー  
...
- **動作の監視**  
  Windows Defender の動作の監視機能を許可または禁止します。 Windows 10 内蔵のこれらのセンサーでは、オペレーティング システムから動作信号を収集して処理し、プライベート クラウドに他から切り離されて存在する Microsoft Defender ATP インスタンスにこのセンサー データを送信します。  
...
詳細については、次を参照してください。 [2.2.2 FW_PROFILE_TYPOE]( https://docs.microsoft.com/openspecs/windows_protocols/ms-fasp/7704e238-174d-4a5e-b809-5f3787dd8acc) Windows プロトコルのドキュメント。  

- **ファイアウォール プロファイルのドメイン**  
...
- **使用可能な場合は、高度なスプーフィング対策を要求する**  
  場合は、デバイスは拡張スプーフィング対策を使用可能な場合に使用します。 If No, スプーフィング対策がブロックされます。 構成されていない、クライアントで構成を優先します。  
...
- **PIN に小文字が必要**  
  必要な場合、ユーザーの PIN は少なくとも 1 つの小文字を含める必要があります。

  **既定値**: 許可

- **PIN に特殊文字が必要**  
  必要な場合、ユーザーの PIN は少なくとも 1 つの特殊文字を含める必要があります。

  **既定値**: 許可

- **PIN の長さの最小値**  
  PIN の最小長は 4 から 127 文字の間である必要があります。

  **既定値**: 6

- **PIN に大文字が必要**  
  必要な場合、ユーザーの PIN は少なくとも 1 つの大文字を含める必要があります。
...
*Spring 2019 更新 (19 H 1) 用のセキュリティ ベースラインの MDM*テンプレートから、次の変更には、*プレビュー*テンプレート。

### <a name="changes-to-the-baseline-settings"></a>ベースラインの設定の変更
以下の設定は、次のいずれかになります。
- *新しい*この最新バージョンの基準にします。
- *削除*この最新のベースライン バージョンからが存在していた以前のバージョン。
- *改訂*何らかの方法から、以前のバージョンの設定の表示にします。 
...
- リムーバブル ドライブのポリシーをロッカーをビット >**暗号化メソッド**
- **Bitlocker でドライブのポリシーを固定** *(すべての設定)*
- **ビットのシステム ドライブのポリシーをロッカー** *(すべての設定)*

" *[新規]* " [**接続**](#connectivity):
- **UNC パスへのアクセスをセキュリティで保護された構成します。**

" *[新規]* " [**Device Guard**](#device-guard):
- **仮想化ベースのセキュリティ**


*[新規]* [ **DMA Guard**](#dma-guard):
- **Kernel DMA Protection と互換性のない外部デバイスの列挙**  
...
- **ファイルのダウンロードに Internet Explorer インターネット ゾーンの自動プロンプト**> 既定値は今すぐ**無効**します。 プレビューで有効に設定されていました。
