...
| **任意のアプリの切り取りとコピーの文字数制限** | 組織のデータとアカウントから切り取りまたはコピーできる文字数を指定します。  これで、 **[他のアプリとの間で切り取り、コピー、貼り付けを制限する]** 設定に関係なく、指定した文字数を任意のアプリケーションと共有できます。<p>既定値 = 0<p>**注**:アプリに Intune SDK バージョン 9.0.14 以降が必要です。  | **0**   |
