...
|**その他のアプリでの Web コンテンツの転送を制限する** | Web リンクを開くことができるアプリの種類を指定します。 <ul><li>あらゆるアプリで Web リンクを開くことを許可する場合、**[任意のアプリ]** を選択します。 </li></ul><ul><li>Web コンテンツはポリシーで管理されているブラウザーでのみ開くことを許可する場合、**[ポリシーで管理されているブラウザー]** を選択します。</li></ul>デバイスの管理に Intune を使用する場合は、[Microsoft Intune での Managed Browser ポリシーを使用するアクセスの管理](app-configuration-managed-browser.md)に関するページを参照してください。 <br><br> モバイル デバイス (iOS と Android) 向けの Microsoft Edge ブラウザーが、Intune アプリ保護ポリシーをサポートします。 Edge ブラウザー アプリケーションで企業の Azure AD アカウントを使用してサインインしたユーザーは、Intune によって保護されます。 Microsoft Edge ブラウザーは MAM SDK を組み込み、そのデータ保護ポリシーをすべてサポートします。ただし、例外として次が禁止されます。<ul><li>**名前を付けて保存**: Microsoft Edge ブラウザーでは、クラウド ストレージ プロバイダー (OneDrive など) にアプリ内で直接接続を追加することがユーザーに許可されません。また、ローカル ファイル システムにファイルをダウンロードすることがユーザーに許可されません。</li><li>**連絡先の同期**: Microsoft Edge ブラウザーによって、ネイティブの連絡先一覧に保存されることはありません。</li></ul><br>Edge ブラウザーでは、Intune ポリシーの自動登録を利用します。 つまり、デバイスの別の Microsoft アプリケーションが Intune ポリシーで管理されているときに、Edge ブラウザーによって、Edge アプリを対象にしているポリシーがあるかどうかが自動的に確認されます。 Intune ポリシーの自動登録は MAM SDK によって処理され、アプリの関与は必要ありません。 <br><br>既定値 = **任意のアプリ** |
...
Intune SDK と Intune アプリ保護ポリシーには、LinkedIn アカウントの接続を管理するためのサポートは含まれていませんが、別の方法でそれらを管理できます。 組織全体に対して LinkedIn アカウントの接続を無効にすることも、組織内の選択したユーザー グループに対して LinkedIn アカウントの接続を有効にすることもできます。 これらの設定は、すべてのプラットフォーム (Web、モバイル、デスクトップ) 上の Office 365 アプリ間の LinkedIn 接続に影響します。 以下の操作を実行できます。
...
|サブスクリプション     |このスコープは利用できず、まだ使用されていません。 Office 365 などの Microsoft アプリとサービスに対する、ユーザーの組織によって提供されるサブスクリプションが含まれます。         |
