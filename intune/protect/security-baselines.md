...
- [プロファイルの作成](#create-the-profile) – 使用する設定を構成して、ベースラインをグループに割り当てます。
...
4. **[Review update]\(更新のレビュー\)** を選択し、プロファイルの現在のインスタンスのバージョンと選択した新しいバージョン間の違いを表示する CSV ファイルをダウンロードします。 このファイルを確認して、どの設定が新規または削除されるのか、また更新されたプロファイルでのこれらの設定の規定値は何かを把握します。  
