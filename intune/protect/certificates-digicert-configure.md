...
3. **[Certificate Connector ソフトウェアをネットワーク上のセキュリティで保護された場所にダウンロード]** を選択します。 インストールするサーバーからアクセスできる場所に、ソフトウェアを保存します。  
...
2. Intune ポータルで、信頼済み証明書プロファイルを作成します。
   ...
   」を参照します。 [Intune](https://go.microsoft.com/fwlink/?linkid=2090973) にサインインします。

   b. **[デバイス構成]**  >  **[管理]**  >  **[プロファイル]**  >  **[プロファイルの作成]** の順に選択します。

   c. 信頼済み証明書プロファイルの **[名前]** と **[説明]** の詳細を入力します。

   d. **[プラットフォーム]** ドロップダウン リストで、この信頼された証明書のデバイス プラットフォームを選択します。

   e. **[プロファイルの種類]** ドロップダウン リストで、 **[信頼済み証明書]** を選択します。

   f. 前のステップで DigiCert CA から取得した信頼されたルート CA の .cer ファイルを参照し、 **[OK]** を選択します。

   g. Windows 8.1 および Windows 10 デバイスの場合のみ、信頼済み証明書の保存先ストアを以下から選択します。    
      - **コンピューター証明書ストア - ルート**  
      - **コンピューター証明書ストア - 中間**  
      - **ユーザー証明書ストア - 中間** 

   h. 完了したら、 **[OK]** を選択し、 **[プロファイルの作成]** ウィンドウに戻り、 **[作成]** を選択します。  
...
3. PKCS 証明書プロファイルの **[名前]** と **[説明]** の詳細を入力します。  

4. **[プラットフォーム]** ドロップダウン リストから、サポートされているデバイス プラットフォームを選択します。

5. **[プロファイルの種類]** ドロップダウン リストで、 **[PKCS 証明書]** を選択します。
 
6. **[PKCS 証明書]** ウィンドウで、次の表の値を使用してパラメーターを構成します。 これらの値は、Intune Certificate Connector を介して DigiCert CA から PKCS 証明書を発行するために必要です。 
...
7. ビジネス ニーズに合わせてプロファイルの構成を完了した後、 **[OK]** を選択してプロファイルを保存します。 

8. **[割り当て]** を選択し、このプロファイルを受け取る適切なグループを構成します。 割り当てるグループには、1 名以上のユーザーまたは 1 台以上のデバイスが含まれている必要があります。
...
| 問題/エラー メッセージ | 解消手順 |
| --- | --- |
| Unable to sign in with the Intune tenant admin account on NDES Connector UI. (NDES Connector UI では Intune テナント管理者アカウントを使用してサインインできません。) | この問題は、Intune 管理ポータルでオンプレミスの Certificate Connector が有効になっていない場合に発生します。 この問題を解決するには、次のいずれかの手順を使用します。 <br><br> Silverlight UI から: <br> 1.[Intune 管理ポータル](https://admin.manage.microsoft.com)にサインインします。 <br> 2. **[管理者]** を選択します。 <br> 3. **[モバイル デバイス管理]**  >  **[Certificate Connector]** の順に選択します。 <br> 4. **[On-premises Certificate Connector の構成]** を選択します。 <br> 5. **[Certificate Connector を有効にする]** チェック ボックスをオンにします。 <br> 6. **[OK]** を選択します。 <br><br> Azure Portal UI の場合: <br> 1.[Azure ポータル](https://portal.azure.com)にサインインします。 <br> 2.Microsoft Intune に移動します。 <br> 3. **[デバイス構成]**  >  **[証明機関]** の順に選択します。 <br> 4. **[有効]** を選択します。 <br><br> Silverlight UI または Azure portal のいずれかで上記の手順を完了してから、同じ Intune テナント管理者アカウントを使用して NDES Connector UI にサインインします。 |
