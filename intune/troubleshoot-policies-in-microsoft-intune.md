...
4. **[デバイス]** で、問題のあるデバイスを探します。 別の列を確認します。
    ...
    - **管理対象**: デバイスがコンプライアンス ポリシーまたは構成ポリシーを受け取るには、このプロパティに **[MDM]** または **[EAS/MDM]** と表示されている必要があります。
      ...
      - アプリ保護ポリシー (モバイル アプリケーション管理) の場合は、デバイスが登録されている必要はありません。 詳細については、「[アプリ保護ポリシーを作成して割り当てる方法](app-protection-policies.md)」を参照してください。
    ...
    - **[Intune 準拠]** : **[はい]** にしてください。 **[いいえ]** と表示される場合は、コンプライアンス ポリシーに問題があるか、またはデバイスが Intune サービスに接続されていない可能性があります。 たとえば、デバイスがオフになっていたり、ネットワークに接続していない場合があります。 最終的に、デバイスは非準拠になります (一般に 30 日後)。
      ...
      詳細については、「[Intune のデバイス コンプライアンス ポリシーの概要](device-compliance-get-started.md)」をご覧ください。
    ...
    - ポリシー固有の情報を表示するデバイスを選択します。
      ...
      **[ポリシーの状態]** :
      ...
      - **エラー**: エラーと考えられる解決策については、「[Microsoft Intune での会社のリソースへのアクセスに関する問題のトラブルシューティング](troubleshoot-company-resource-access-problems.md)」をご覧ください。

      **役に立つリンク**: 

      - [デバイス コンプライアンス ポリシーを展開する方法](device-compliance-get-started.md#ways-to-deploy-device-compliance-policies)
...
**役に立つリンク**: [デバイス構成プロファイルを監視する](device-profile-monitor.md)
...
「[Intune のオンプレミス Exchange Connector のトラブルシューティング](troubleshoot-exchange-connector.md)」が役に立つ場合があります。
