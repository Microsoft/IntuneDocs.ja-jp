...
3. **[追加]** をクリックして、 **[要件規則の追加]** ウィンドウを表示し、追加の要件規則を構成します。 **[要件の種類]** を選択し、要件の検証方法を決定するために使用する規則の種類を選びます。 要件規則は、ファイル システム情報、レジストリ値、または PowerShell スクリプトをベースにすることができます。 
