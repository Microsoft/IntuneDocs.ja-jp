...
## <a name="soft-transitions-from-work-to-personal-accounts"></a>仕事用から個人用アカウントへの論理遷移

Microsoft Edge モバイル エンタープライズ エクスペリエンスの基礎はデュアル ID モデルであり、Microsoft Edge では仕事用と個人用の両方の ID がサポートされることを意味します。 Office 365 や Outlook アプリと同様に、このデュアル ID モデルでは、エンド ユーザーがすべての参照ニーズに Microsoft Edge を使用し、管理者によって定義されたコンテンツ ポリシーに基づく 2 つのエクスペリエンス間を簡単に移動できます。 個人用コンテキスト内での参照は影響を受けず、企業の情報は Microsoft Edge 内の仕事用コンテキストに完全に格納された状態になります。 

このモデルの利点の 1 つは、ユーザーが組織では許可されていないサイトへのリンク (新聞記事など) を開こうとした場合、個人用コンテキストでは実行でき、仕事用コンテキストからの完全な分離が維持されることです。 これらの論理遷移は、既定で有効になっています。 

Microsoft Edge または Managed Browser アプリの構成を作成する手順に従い、以下のキーと値のペアを指定します。

| キー                                                                | 値                                                 |
|--------------------------------------------------------------------|-------------------------------------------------------|
| **com.microsoft.intune.mam.managedbrowser.AllowTransitionOnBlock** | **False** にすると、これらの論理遷移の実行がブロックされます |
