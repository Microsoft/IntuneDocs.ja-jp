...
ms.locfileid: "66749925"
---
# <a name="windows-security-baseline-settings-for-intune"></a>Intune 用の Windows セキュリティ ベースラインの設定  

Microsoft Intune でサポートされている [Windows セキュリティ ベースラインの設定](security-baselines.md)を確認します。 Windows ベースラインの既定値は、Windows の推奨される構成を表しており、他のセキュリティ ベースラインのベースラインの既定値と一致しない場合があります。  

> [!NOTE]  
> Windows セキュリティ ベースラインの設定は**プレビュー**段階です。 プレビュー段階、使用可能な設定の一覧、およびこのコンテンツが表示するこれらの設定の順序は、ポータルで使用できることにより異なります。  
>  
