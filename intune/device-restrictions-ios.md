...
- **[使用状況データの共有]**: **[ブロック]** を選択すると、診断および使用状況データがデバイスによって Apple に送信されません。 **[未構成]** (既定値) では、このデータの送信が許可されます。
  - **診断の送信の設定変更 (監視モードのみ)**:**ブロック**で診断送信とアプリ分析の設定を変更できなくなりますユーザー**診断と使用状況**(デバイスの設定)。 **[未構成]** (既定値) では、ユーザーがデバイスのこれらの設定を変更できるようになります。

    この機能は、以下に適用されます。  
    - iOS 9.3.2 以降

- **[画面の取り込み]**: **[ブロック]** を選択すると、デバイス上でスクリーンショットまたは画面の取り込みを実行できなくなります。 **[未構成]** (既定値) では、ユーザーが画面のコンテンツを画像としてキャプチャできるようになります。
  - **[Classroom アプリによるリモート画面の監視 (監視のみ)]**: **[ブロック]** を選択すると、Classroom アプリがリモート デバイス上の画面を監視できなくなります。 **[未構成]** (既定値) では、Apple Classroom アプリが画面を監視できるようになります。

    この機能は、以下に適用されます。  
    - iOS 9.3 以降

  - **[Classroom アプリによるプロンプトなしの画面監視 (監視モードのみ)]**: **[許可]** に設定すると、学生に知られることなく Classroom アプリを使用して学生の iOS デバイスの画面を黙って監視することができます。 学生のデバイスが Classroom アプリを使用してクラスに登録されると、その学科の教師にアクセス許可が自動的に付与されます。 **構成されていない**(既定値) は、この機能を防止します。
- **[信頼できない TLS 証明書]**: **[ブロック]** を選択すると、信頼されていないトランスポート層セキュリティ (TLS) 証明書がデバイスで禁止されます。 **構成されていない**(既定値) が TLS 証明書を許可します。
...
- **[アカウントの変更 (監視モードのみ)]**: **[ブロック]** を選択すると、ユーザーが iOS 設定アプリでデバイス固有の設定を更新できません。 たとえば、ユーザーは、新しいデバイス アカウントを成したり、ユーザー名またはパスワードを変更したりすることができません。 **[未構成]** (既定値) では、ユーザーがこれらの設定を変更できるようになります。

  この機能は、メール、連絡先、予定表、Twitter など、iOS 設定アプリからアクセスできる設定にも適用されます。 Microsoft Outlook アプリなど、アカウント設定が iOS 設定アプリから構成できないアプリには適用されません。
- **時間 (監視モードのみ) を画面**: 選択**ブロック**をユーザーが画面の時間 (デバイスの設定) で独自の制限を設定するを防ぐためにします。 **構成されていない**デバイス上のデバイス制限 (保護者による制限や、コンテンツやプライバシーの制限) を構成するユーザーを許可します。

  この設定から名前が変更された**デバイス設定で制限を有効にする**します。 この変更の影響:  
  
  - iOS 11.4.1 以前: **[ブロック]** は、エンド ユーザーがデバイス設定で独自の制限を設定できないようにします。 これは同じです。エンドユーザーの変更はありません。
  - iOS 12.0 以降:**ブロック**エンドユーザーが独自に設定するを防ぎます**画面時間**デバイス設定で (設定 > [全般] > 画面時間)、コンテンツとプライバシーの制限など。 デバイスが iOS 12.0 にアップグレードでは、デバイスの設定での制限 タブをもう表示されません (設定 > 全般 > デバイスの管理 > 管理プロファイル > の制限)。 これらの設定は **[画面の表示時間]** にあります。
  
- **[デバイスでの [すべてのコンテンツと設定を消去] オプションの使用 (監視モードのみ)]**: **[ブロック]** を選択すると、ユーザーはデバイスのすべてのコンテンツと設定を消去するオプションを使用できなくなります (監視モードのみ)。 **[未構成]** (既定値) では、ユーザーがこれらの設定を使用できるようになります。
- **[デバイス名の変更 (監視モードのみ)]**: **[ブロック]** を選択すると、デバイス名を変更できなくなります。 **[未構成]** (既定値) では、ユーザーがデバイスの名前を変更できるようになります。
- **[通知設定の変更 (監視モードのみ)]**: **[ブロック]** を選択すると、通知設定を変更できなくなります。 **[未構成]** (既定値) では、ユーザーがデバイスの通知設定を変更できるようになります。
- **[壁紙の変更 (監視モードのみ)]**: **[ブロック]** では、壁紙を変更できなくなります。 **[未構成]** (既定値) では、ユーザーがデバイスの壁紙を変更できるようになります。
- **[エンタープライズ アプリの信頼設定の変更 (監視モードのみ)]**: **[ブロック]** では、監視対象のデバイスでユーザーがエンタープライズ アプリの信頼設定を変更できなくなります。 **[未構成]** (既定値) では、アプリ ストアからダウンロードされていないアプリをユーザーが信頼できます。
- **[構成プロファイルの変更 (監視モードのみ)]**: **[ブロック]** では、デバイスでの構成プロファイルの変更が禁止されます。 **[未構成]** (既定値) では、ユーザーが構成プロファイルをインストールできるようになります。
- **[アクティベーション ロック (監視下のみ)]**: **[許可]** を選択すると、監視されている iOS デバイスでアクティベーション ロックが有効になります。 アクティベーション ロックにより、紛失したり盗難にあったデバイスの再アクティブ化が困難になります。
- **[アプリ削除をブロックする (監視モードのみ)]**: **[ブロック]** を選択すると、ユーザーがアプリを削除できなくなります。 **[未構成]** (既定値) では、ユーザーがデバイスからアプリを削除できるようになります。
- **[Blocks USB Restricted mode (supervised only)]\(USB 制限モードをブロックする (監視モードのみ)\)**: **[ブロック]** を選択すると、監視対象のデバイスで USB 制限モードが無効になります。 USB 制限モードでは、USB アクセサリが 1 時間を超えてロックされているデバイスとデータ交換できないようにします。 **構成されていない**により、USB の制限モード (既定値)。
...
- **[Classroom の学科をやめる学生は許可を求めることが必要 (監視モードのみ)]**: **[必要]** では、Classroom アプリを使用して非管理の学科に登録している学生は、その学科をやめるために教師に許可を求める必要があります。 **[未構成]** (既定値) では、学生は許可を求めるように強制されません。

  この機能は、以下に適用されます。  
  - iOS 11.3 以降

- **Classroom アプリにロック (監視モードのみ) メッセージを表示せずに、デバイスをロックを許可する**:**を有効にする**アプリをロックまたは受講者の入力を求めることがなく Classroom アプリを使用してデバイスをロックする、教師を許可します。 アプリの手段をロック、デバイスでは、指定されたアプリのアクセスの教師のみをできます。 **構成されていない**(既定値) は、アプリやデバイスの Classroom アプリを使用して、学生のメッセージを表示せずにロックから教師を防止します。 

  この機能は、以下に適用されます。  
  - iOS 11.0 以降

- **(監視モードのみ) の入力を求めることがなく Classroom のクラスが自動的に結合**:**を有効にする**学生、教師のメッセージを表示せず、Classroom アプリに含まれるクラスの結合にできるようにします。 **構成されていない**(既定値) には、受講者が Classroom アプリ内にあるクラスを結合する教師がメッセージが表示されます。

  この機能は、以下に適用されます。  
  - iOS 11.0 以降

- **[無線による PKI の更新を許可する]**: **[許可]** を選択すると、ユーザーはデバイスをコンピューターに接続せずにソフトウェア更新プログラムを受信できるようになります。
- **[広告トラッキングを制限する]**: **[制限]** を選択すると、デバイスの広告識別子が無効になります。 **構成されていない**(既定値) が有効になっていることを保持します。
- **[VPN の作成をブロックする (監視モードのみ)]**: **[ブロック]** では、ユーザーが VPN 構成の設定を作成できなくなります。 **[未構成]** (既定値) では、ユーザーがデバイス上で VPN を作成できます。
- **Esim 状の設定 (監視モードのみ) を変更する**:**ブロック**ユーザーが削除または移動体通信計画を追加するデバイスの esim 状にできないようにします。 **[未構成]** (既定値) では、ユーザーがこれらの設定を変更できるようになります。

  この機能は、以下に適用されます。  
  - iOS 12.1 以降

- **ソフトウェアの更新 (監視モードのみ) 遅延**: に設定すると**未構成**(既定)、ソフトウェア更新プログラムがデバイスに表示される、Apple によってリリースされるとします。 たとえば、iOS 更新プログラムは、Apple によって、特定の日付にリリースを取得、し、その更新プログラムを自然に表示リリース日に、デバイス。
  ...
  **有効にする**ソフトウェア更新プログラムが 0 ~ 90 日からのデバイスに表示されるときに遅延することができます。 更新プログラムが、またはインストールされていない、この設定を制御しません。 
  ...
  - **ソフトウェア更新プログラムの可視性を遅延**: 0 ~ 90 日間の値を入力します。 遅延期間が経過すると、ユーザーは、遅延がトリガーされた時点で利用可能な OS の最も古いバージョンに更新する通知を受け取ります。
    ...
    たとえば場合、iOS 12.a は**1 月 1 日**と**可視性を遅延**に設定されている**5 日間**、し、iOS 12.a はエンド ユーザー デバイスで使用可能な更新プログラムとして表示されません。 **6 日**次のリリースでは、更新プログラムがあるし、エンドユーザーがインストールできることです。
...
- **[パスコードの変更 (監視モードのみ)]**: **[ブロック]** を選択すると、パスコードの変更、追加、削除が停止されます。 この機能をブロックすると、監視対象デバイスではパスコードの変更制限は無視されます。 **[未構成]** (既定値) では、パスコードの追加、変更、または削除が許可されます。

  - **[指紋の変更 (管理モードのみ)]**: **[ブロック]** を選択すると、ユーザーによる TouchID の指紋の変更、追加、または削除が停止されます。 **[未構成]** (既定値) では、デバイスで TouchID の指紋をユーザーが更新できるようになります。
...
- **Touch ID または Face ID の認証パスワードやクレジット_カード情報 (監視モードのみ) 自動入力を要求する**: に設定すると**必要**ユーザーがパスワードやクレジット_カードの前に touch Id または FaceID を使用して認証する必要があります情報は、自動入力 Safari と他のアプリを指定できます。 **構成されていない**(既定値) をデバイスの設定では、この機能を制御できます。
...
- **[管理対象ではないアプリで会社のドキュメントを表示]**: **[ブロック]** では、管理対象ではないアプリで社外ドキュメントを表示できなくなります。 **[未構成]** では、すべてのアプリで会社のドキュメントを表示できるようになります。 たとえば、ユーザーが OneDrive アプリから Dropbox にファイルを保存できないようにしたい場合です。 この設定を **[ブロック]** として構成します。 デバイスがポリシーを受信した後 (たとえば、再起動後)、保存は許可されなくなります。
  - **書き込みの非管理対象の連絡先アカウントに連絡先を管理対象アプリを許可する**: に設定すると**許可**ユーザーが追加または同期に、ビジネスと企業の連絡先を含む任意のユーザーの Outlook 連絡先情報、デバイスの組み込みの連絡先アプリ。 **[未構成]** に設定すると、ユーザーは Outlook 連絡先をデバイス上の組み込みの連絡先アプリに追加できなくなります。
...
- **[社内アプリでの社外ドキュメントの表示]**: **[ブロック]** では、社内アプリで社外ドキュメントを表示できなくなります。 **[未構成]** では、会社の管理対象アプリですべてのドキュメントを表示できるようになります。
  - **非管理対象アプリ管理対象の連絡先アカウントからの読み取りを許可する**: に設定すると**許可**ユーザーが Outlook にすべてのユーザーの iContacts アプリの連絡先情報を追加できます。 **[未構成]** では、デバイス上の組み込みの連絡先アプリから、重複の削除を含む読み取りができなくなります。
...
- **携帯電話のプランの設定 (監視モードのみ) の変更を**:**ブロック**移動体通信の計画のすべての設定を変更できないようにします。 **構成されていない**(既定値) がユーザーに変更を許可します。

  この機能は、以下に適用されます。  
  - iOS 11.0 以降

- **[個人用ホットスポット]**: **[ブロック]** では、デバイスが個人用ホットスポットとして使用されないようになります。 この設定は、一部の通信事業者とは互換性がない可能性があります。 **構成されていない**(既定値) は、この機能を使用します。
...
- **近くのデバイス (監視モードのみ) 新しい構成設定をブロック**:**ブロック**近くに新しいデバイスをセットアップするプロンプトを無効にします。 **構成されていない**により近くにあるその他の Apple デバイスに接続するユーザーにプロンプトが、(既定値)。
...
- **[キオスク モードで実行するアプリ]**: キオスク モードで実行するアプリの種類を選択します。 次のようなオプションがあります。
  - **構成されていない**: キオスクの設定は適用されません。 デバイスがキオスク モードで実行されません。
  - **[ストア アプリ]**: iTunes App ストアでのアプリの URL を入力します。
  - **[管理対象アプリ]**: Intune に追加したアプリを選択します。
  - **組み込みアプリ**: 入力、[バンドル ID](#bundle-ids-for-built-in-ios-apps) (この記事では) での組み込みアプリ。
...
- **マークされていないメール ドメイン** > **メール ドメイン URL**: 1 つまたは複数の Url を一覧に追加します。 入力したドメイン以外のドメインからのメールをエンド ユーザーが受信すると、そのメールは iOS のメール アプリで信頼されていないメールとしてマークされます。

- **管理された web ドメイン** > **Web ドメイン URL**;一覧に 1 つまたは複数の Url を追加します。 入力したドメインからダウンロードされたドキュメントは、管理されているものと見なされます。 この設定は、Safari ブラウザーを使用してダウンロードされたドキュメントにのみ適用されます。

- **Safari パスワードの自動入力ドメイン** > **ドメイン URL**: 1 つまたは複数の Url を一覧に追加します。 ユーザーは、この一覧の URL からの Web パスワードのみを保存できます。 この設定は、Safari ブラウザー、および iOS 9.3 以降の監視モードのデバイスにのみ適用されます。 URL を指定しないと、すべての Web サイトからのパスワードが保存されます。
...
デバイスの機能と設定を制限することも[macOS](device-restrictions-macos.md)デバイス。