...
- **[画面の取り込み]**: **[ブロック]** を選択すると、デバイス上でスクリーンショットまたは画面の取り込みを実行できなくなります。 IOS 9.0 以降では、これも含まれ画面記録をブロックしています。 **[未構成]** (既定値) では、ユーザーが画面のコンテンツを画像またはビデオとしてキャプチャできるようになります。
...
- **個人用ホット スポット**:**ブロック**デバイス同期を毎回ユーザーのデバイスの個人用ホット スポットをオフにします。この設定は、一部の通信事業者とは互換性がない可能性があります。 **構成されていない**(既定値) は、既定のユーザー セットとして個人用のホット スポットの構成を保持します。
