...
5. 一覧の上の **[追加]** を選択します。
6. 一覧の使用可能な **[ストア アプリ]** の種類から **[Windows]** を選択します。
7. **[アプリ情報]** を選択して、アプリ情報を構成します。
8. **[アプリ情報]** ブレードで、次の情報を構成します。選択したアプリによっては、このウィンドウの一部の値が自動的に入力される場合があります。
    ...
    - **[カテゴリ]** (省略可能) - 1 つ以上のアプリ カテゴリ、または作成したカテゴリを選択します。 この情報を追加すると、ユーザーは会社のポータルを参照する際にアプリを見つけやすくなります。
    ...
    - **[アップロード アイコン]** - アプリに関連付けるアイコンをアップロードします。 ユーザーが会社のポータルを参照するときに、アプリにこのアイコンが表示されます。
1. 必要なアプリ情報をすべて入力したら、**[OK]** をクリックします。
2. **[アプリの追加]** ブレードで **[追加]** をクリックします。
...
3. **プラットフォーム**として **[Windows 10 all devices (Windows 10 のすべてのデバイス)]** を選択し、適切な**アーキテクチャ**を選択してダウンロードします。 このアプリは、アプリ ライセンス ファイルを必要としません。
    ...
    ![ダウンロードする Windows 10 のすべてのデバイスとアーキテクチャ X86 のパッケージの詳細のイメージ](./media/Win10CP-all-devices.png)
...
5. ポータル サイト アプリを Intune にアップロードする前に、パッケージを次のように構成してフォルダー (C:&#92;ポータル サイトなど) を作成します。
  ...
  - 依存関係フォルダーに 9 つの依存関係パッケージを配置します。 依存関係がこの形式で配置されていない場合、Intune はパッケージのアップロード時にこれらを認識してアップロードできなくなり、次のエラーでアップロードが失敗します。 
