...
description: Android エンタープライズ デバイスまたは Android for Work デバイスで、デバイスでの設定を制限します。これには、コピーと貼り付け、通知の表示、アプリのアクセス許可、データの共有、パスワードの長さ、サインイン エラー、指紋によるロック解除、パスワードの再利用、勤務先の連絡先の Bluetooth 共有の有効化が含まれます。 アプリを 1 つまたは複数のアプリを実行する専用のデバイスのキオスクとしてデバイスを構成します。
keywords: ''
...
- **アプリの脅威のスキャン**:**必要**前に、と後、インストールされているアプリをスキャンする Google Play Protect を有効 (既定値)。 脅威を検出した場合は、デバイスからアプリを削除するユーザーを警告する可能性があります。 **構成されていない**有効または Google Play Protect アプリのスキャンを実行しません。

### <a name="dedicated-device-settings"></a>専用のデバイスの設定

専用デバイスでキオスク スタイルのエクスペリエンスを構成するのにには、これらの設定を使用します。 デバイスを構成して、1 つのアプリを実行させたり、複数のアプリを実行させたりすることができます。 デバイスがキオスク モードで設定されているときは、自分が追加したアプリのみを使用できます。 これらの設定は、専用の Android エンタープライズ デバイスに適用されます。 これらは、完全に管理された Android Enterprise デバイスに適用されません。

**キオスク モード**: デバイスが 1 つのアプリを実行または複数のアプリを実行するかどうかに選択します。
...
- **[単一アプリ]**: ユーザーはデバイスで 1 つのアプリにのみアクセスできます。 デバイスを起動させると、特定のアプリのみが起動されます。 ユーザーは新しいアプリを開いたり、実行中のアプリを変更したりできません。
  ...
  **手順**
  1. **[マネージド アプリを選択します]** を選択し、一覧から Google Play のマネージド アプリを選択します。 
      ...
      アプリが一覧表示されない場合は、デバイスに[いくつかの Android アプリを追加](apps-add-android-for-work.md)します。 必ず[専用デバイス用に作成したデバイス グループにアプリを割り当てます](apps-deploy.md)。
...
- **[複数アプリ]**: ユーザーはデバイスで限られた一連のアプリにアクセスできます。 デバイスを起動させると、ご自身で追加したアプリのみが起動されます。 ユーザーが開くことができる Web リンクをいくつか追加することもできます。 ポリシーが適用されると、ユーザーのホーム画面に許可されたアプリのアイコンが表示されます。
  ...
  >   - 専用デバイス用に作成した[デバイス グループに割り当てられている](apps-deploy.md)
  > 
  ...
  - **[追加]** を選択して、一覧からアプリを選択します。
    ...
    **Managed Home Screen** アプリが一覧表示されない場合は、[これを Google Play から追加します](https://play.google.com/work/apps/details?id=com.microsoft.launcher.enterprise)。 必ず専用デバイス用に作成したデバイス グループに[アプリを割り当てます](apps-deploy.md)。

    また、ご自身の組織が作成したその他の [Android アプリ](apps-add-android-for-work.md)や [Web アプリ](web-app.md)をデバイスに追加することもできます。 必ず[専用デバイス用に作成したデバイス グループにアプリを割り当てます](apps-deploy.md)。

  - **[仮想ホーム ボタン]**: **[有効化]** を選択すると、専用デバイス上にホーム ボタンが表示されるようになります。 これを選択するとユーザーはデバイスのホーム画面に戻るため、ユーザーがアプリ間を切り替えやすくなります。 一部の Android デバイス上では、ユーザーがホーム ボタンを表示するために、画面の上方向にスワイプする必要がある場合があります。 **[無効にする]** ではホーム ボタンが表示されなくなるため、ユーザーはアプリ間を切り替えるために [戻る] ボタンを使う必要があります。
  - **[キオスク モードを終了する]**: **[有効化]** を選択すると、デバイスを更新するために管理者がキオスク モードを一時停止できるようになります。 管理者は、この機能を使用するには。 
  ...
  - **[カスタム URL の背景の設定]**: 専用デバイス上の背景画面をカスタマイズするための URL を入力します。
    ...
    > [!NOTE]
    > ほとんどの場合、少なくとも次のサイズでのイメージで始まるお勧めします。
    >
    > - 電話番号: 1080 x 1920 px
    > - タブレット: 1920 x 1080 ピクセル
    >    
    > 最適なエクスペリエンスと鮮明な詳細については、デバイスのイメージごとの資産は、ディスプレイの仕様を作成するをお勧めします。
    >
    > 最新表示より高いピクセル密度を持ち、対応 2 K/4 K の定義のイメージを表示できます。
  - **Wi-fi 構成**: 選択**を有効にする**エンドユーザーにデバイスを別の WiFi ネットワークに接続できるようにします。 この機能を有効にするデバイスの場所にもになります。 **構成されていない**(既定値) が管理されているホーム画面で (ロック タスク モード) の WiFi ネットワークに接続できないようにします。

    詳細[ロック タスク モード](https://developer.android.com/work/dpc/dedicated-devices/lock-task-mode)(Android の web サイトを開きます)。

  - **Bluetooth 構成**: 選択**を有効にする**デバイスで Bluetooth を許可して、Bluetooth 経由でエンドユーザーのペアのデバイスをできるようにします。 この機能を有効にするデバイスの場所にもになります。 **構成されていない**(既定値) には、ユーザーが Bluetooth を構成および管理されているホーム画面で (ロック タスク モード) のデバイスをペアリングができないようにします。 
...
- **ロック画面の機能を無効になっている**: キーガードがデバイスで有効な場合は、無効にする機能を選択します。 たとえば、**[セキュリティで保護されたカメラ]** がオンの場合、デバイスのカメラ機能は無効になります。 オンではない機能はすべてデバイスで有効になります。

  これらの機能は、デバイスがロックされているときにユーザーが使用できます。 ユーザーを参照してくださいまたはオンになっている機能にアクセスしません。

- **[必要なパスワードの種類]**: デバイスに必要なパスワードの種類を定義します。 次のようなオプションがあります。
  - **デバイスの既定**
  - **必要に応じて、パスワードの制限なし**
  - **生体認証 weak**:[厳密弱生体認証と](https://android-developers.googleblog.com/2018/06/better-biometrics-in-android-p.html)(Android の web サイトを開きます)
  - **数値**: パスワードの番号をなどあるのみ必要があります`123456789`します。 **[パスワードの最小文字数]**: ユーザーが入力する必要がある最小文字数 (4 - 14 文字) を入力します。
  - **数値複素数**: 繰り返しや連続の番号 ("1111" や "1234" など) を使用できなくなります。 **[パスワードの最小文字数]**: ユーザーが入力する必要がある最小文字数 (4 - 14 文字) を入力します。
  - **アルファベット**: アルファベット文字が必要です。 数字および記号は必要ありません。 **[パスワードの最小文字数]**: ユーザーが入力する必要がある最小文字数 (4 - 14 文字) を入力します。
  - **英数字**: 大文字、英小文字、および数値文字が含まれています。 **[パスワードの最小文字数]**: ユーザーが入力する必要がある最小文字数 (4 - 14 文字) を入力します。
  - **英数字と記号**: 大文字、小文字、数字、句読点、およびシンボルが含まれています。 次の項目も入力します。

    - **パスワードの最小長**: 4 ~ 16 文字のパスワードがいる必要があります最小長を入力します。
    - **必要な文字数**: 0 ~ 16 文字の間、パスワード必要がありますの文字の数を入力します。
    - **必要な小文字の文字数**: パスワードが必要小文字の文字の 0 ~ 16 文字の範囲の番号を入力します。
    - **必要な大文字の文字数**: パスワードが必要大文字の文字の 0 ~ 16 文字の範囲の番号を入力します。
    - **必要なアルファベット以外の文字数**: 番号を入力します (アルファベット文字以外のもの) 以外の文字のパスワードが必要、0 ~ 16 文字の範囲。
    - **必要な数字の数**: の文字の数を入力します (`1`、 `2`、`3`など) 0 ~ 16 文字の間、パスワードである必要があります。
    - **必要なシンボルの文字数**: 記号の数を入力します (`&`、 `#`、`%`など) 0 ~ 16 文字の間、パスワードである必要があります。

- **パスワードの有効期限が切れるまでの日数**: 1 ~ 365 日、デバイスのパスワードを変更する必要がありますまでの間の日数の数を入力します。 たとえば、60 日後に、パスワードを変更する次のように入力します。`60`します。 パスワードの期限が切れると、ユーザーが新しいパスワードを作成するように求められます。
- **ユーザーの前に必要なパスワードの数は resuse パスワード**: 再利用できない、1 ~ 24 の間で最近使用したパスワードの数を入力します。 この設定を使用して、以前に使用されたことのあるパスワードの作成を制限するかどうかを指定します。
...
- **[提供元不明のアプリのインストールを許可する]**: **[許可]** を選択すると、ユーザーが **[不明なソース]** をオンにできるようになります。 この設定は、Google Play ストア以外のソースを含む、不明なソースからインストールするアプリを使用できます。 **[未構成]** では、ユーザーが **[不明なソース]** をオンにできなくなります。
- **Google Play ストアでのすべてのアプリへのアクセスを許可する**: に設定すると**許可**ユーザーが Google Play ストアですべてのアプリへのアクセスを取得します。 ブロックを管理者のアプリへのアクセスが届かなければ[クライアント アプリ](apps-add-android-for-work.md)します。 **構成されていない**強制的にユーザーにのみアクセス管理者が使用可能な Google Play ストア アプリまたはアプリで必要な[クライアント アプリ](apps-add-android-for-work.md)します。
