...
- **[パスワード]** : エンド ユーザーがデバイスにアクセスする際にパスワードの入力を**要求**します。 **[未構成]** (既定値) にすると、パスワードなしでのデバイスへのアクセスが許可されます。 ローカル アカウントのみに適用されます。 ドメイン アカウントのパスワードは、Active Directory (AD) と Azure AD で構成されているままです。
...
- **[毎日のクイック スキャンを実行する時刻]** : 毎日のクイック スキャンを実行する時間を選びます。 **[未構成]** では毎日のスキャンは実行されません。 さらにカスタマイズが必要な場合は、 **[実行するシステム スキャンの種類]** の設定を構成します。
  ...
  > [!WARNING]
  > Azure portal での Intune のこの設定で、状態が [失敗] と表示されることがあります。 これはレポート機能のバグです。 Intune 製品グループは、動作を再現してトラブルシューティングした後、状態が実際は [成功] であることを確認しています。 このレポートのバグは、今後のリリースで修正される予定です。 タイムラインが変更されているため、最新の ETA はありません。 この機能に対するすべての更新は、「[In development for Microsoft Intune (Microsoft Intune の開発)](in-development.md)」で告知されます。
