...
### <a name="use-microsoft-edge-kiosk-mode"></a>Microsoft Edge のキオスク モードの使用
...
- **アイドル時間の後にブラウザーを更新して**: 0 ~ 1440 からブラウザーが更新されるまでのアイドル状態の分単位の数を入力分。 既定値は`5`分。 設定すると`0`(ゼロ)、ブラウザーがアイドル状態に更新されません。
  ...
  この設定で実行する場合にのみ使用可能な[パブリック InPrivate 閲覧 (シングル アプリ キオスク)](#use-microsoft-edge-kiosk-mode)します。
...
- **検索エンジンへの変更を許可する**:**はい**(既定値) が新しい検索エンジンを追加または Microsoft Edge での既定の検索エンジンを変更できます。 選択**いいえ**をユーザーが検索エンジンをカスタマイズすることを防ぐためにします。

  この設定で実行する場合にのみ使用可能な[通常モード (マルチ アプリ キオスク)](#use-microsoft-edge-kiosk-mode)します。
