...
- **[ロック画面の画像の URL (デスクトップのみ)]** - Windows のロック画面の壁紙として使用する、PNG、JPG、または JPEG 形式の画像への URL を指定します。 ユーザーはこれを変更できません。
...
-   **[About Flags へのアクセスをブロック]** - エンド ユーザーが Edge で開発者向けと実験用の設定を含む about:flags ページにアクセスするのを防止します。
