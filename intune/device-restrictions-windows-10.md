...
ms.locfileid: "52728771"
---
# <a name="windows-10-and-newer-device-restriction-settings-in-microsoft-intune"></a>Microsoft Intune 内での Windows 10 (およびそれ以降) のデバイス制限設定

この記事では、Windows 10 デバイスに対して構成できるすべてのデバイス制限設定を説明します。 これらの設定をデバイスの構成プロファイルに追加し、その後デバイスに割り当てたり、デプロイしたりするには、Microsoft Intune を使用します。
...
- **[Screen capture (mobile only)]\(画面キャプチャ (モバイルのみ)\)**: ユーザーがデバイスの画面を画像としてキャプチャできるようにします。
- **[Copy and paste (mobile only)]\(コピー/貼り付け (モバイルのみ)\)**: デバイス上のアプリ間でのコピー/貼り付け操作を許可します。
...
- **[USB connection (mobile only)]\(USB 接続 (モバイルのみ)\)**: デバイスが USB 接続を介して外部記憶装置にアクセスできるかどうかを制御します。
- **[AntiTheft mode (mobile only)]\(盗難防止モード (モバイルのみ)\)**: Windows の盗難防止モードを有効にするかどうかを構成します。
- **[Cortana]**: Cortana による音声アシスタントを有効または無効にします。
- **[Voice recording (mobile only)]\(音声録音 (モバイルのみ)\)**: デバイスのボイス レコーダーの使用を許可またはブロックします。
...
- **[Automatic acceptance of the pairing and privacy user consent prompts]\(ペアリングとプライバシーに関するユーザーの同意プロンプトの自動受け入れ\)**: アプリの実行時にペアリングとプライバシーに関する同意メッセージを自動的に受け入れることを Windows に許可します。
...
- **[Cortana on locked screen (desktop only)]\(ロック画面での Cortana (デスクトップのみ)\)**: デバイスがロック画面になっているとき、Cortana の操作をユーザーに許可しません (Windows 10 デスクトップのみ)。
- **[Toast notifications on locked screen]\(ロック画面でのトースト通知\)**: デバイス ロック画面でアラート メッセージの表示をブロックします。
- **[スクリーン タイムアウト (モバイルのみ)]**: 画面ロック後、画面がオフになるまでの時間を秒単位で指定します。

## <a name="app-store"></a>アプリ ストア

- **[App store (mobile only)]\(アプリ ストア (モバイルのみ)\)**: Windows 10 Mobile デバイスでのアプリ ストアの使用を許可またはブロックします。
...
- **[Shared user app data]\(共有ユーザー アプリ データ\)**: 同じデバイス上の異なるユーザー間でのデータの共有をアプリに許可します。
- **[Use private store only]\(プライベート ストアのみを使用します\)**: 有効にすると、エンド ユーザーがプライベート ストアからアプリをダウンロードすることのみ許可します。
- **[Store originated app launch]\(ストアから配信されたアプリの起動\)**: デバイスにプレインストールされた、または Microsoft ストアからダウンロードされたすべてのアプリを無効にするのに使用します。
- **[Install app data on system volume]\(システム ボリューム上でアプリ データをインストールします\)**: アプリがデバイスのシステム ボリューム上にデータを格納しないようにします。
- **[Install apps on system drive]\(システム ドライブ上でアプリをインストールします\)**: アプリがデバイスのシステム ドライブ上にデータを格納しないようにします。
...
- **[User can change Start pages]\(ユーザーがスタート ページを変更可能\)**: **[許可]** を選択すると、ユーザーがスタート ページを変更できます。 管理者は `EdgeHomepageUrls` を使用して、Microsoft Edge を開いたときに既定でユーザーに表示されるスタート ページを入力できます。 **[Not configured]\(未構成\)** を選択すると、ユーザーはスタート ページを変更できなくなります。
...
- **[Home button]\([ホーム] ボタン\)**: [ホーム] ボタンを選択したときに実行される処理を選択します。 次のようなオプションがあります。
  - **[スタート ページ]**: **[Start Microsoft Edge with]\(Microsoft Edge 起動時のページ\)** 設定で選択したオプションが開きます。
...
- **[ポップアップ]**: **[ブロック]** を選択すると、ブラウザーでポップアップ ウィンドウが表示されなくなります。 Windows 10 デスクトップのみに適用されます。 **[Not configured]\(未構成\)** を選択すると、Web ブラウザーでポップアップが表示されます。
- **[Send intranet traffic to Internet Explorer]\(Internet Explorer にイントラネット トラフィックを送信する\)**: **[許可]** を選択すると、ユーザーは、Microsoft Edge ではなく、Internet Explorer でイントラネット Web サイトを開くことができます (Windows 10 デスクトップのみ)。 **[Not configured]\(未構成\)** を選択すると、ユーザーは Microsoft Edge を使用できます。
- **[Enterprise mode site list location]\(エンタープライズ モードのサイト一覧の場所\)**: エンタープライズ モードで開く Web サイトの一覧を含む URL を入力します。 この一覧はユーザーが変更できません。 Windows 10 デスクトップのみに適用されます。
- **[Message when opening sites in Internet Explorer]\(Internet Explorer でサイトを開くときにメッセージを表示する\)**: この設定を使用すると、Internet Explorer 11 でサイトを開く前に Microsoft Edge で通知を表示するように構成できます。 次のようなオプションがあります。
  - **[Not configured]\(未構成\)**: OS の既定の動作が使用され、メッセージが表示される場合も、表示されない場合もあります。
...
- **[Microsoft 互換性リスト]**: **[ブロック]** を選択すると、Microsoft Edge 内で Microsoft 互換性リストが使用されません。 Microsoft が提供するこのリストを使用すると、既知の互換性の問題があるサイトを Edge で正しく表示できます。 **[Not configured]\(未構成\)** を選択すると、Microsoft 互換性リストを使用できます。
- **[スタート ページと新しいタブ ページの事前読み込み]**: **[ブロック]** を選択すると、スタート ページと新しいタブ ページが事前に読み込まれなくなります。 事前読み込みを行うと、Microsoft Edge の起動時間と新しいタブの読み込み時間が最小限に抑えられます。**[Not configured]\(未構成\)** を選択すると、OS の既定の動作が使用され、これらのページが事前に読み込まれる場合も、読み込まれない場合もあります。
- **[Prelaunch Start pages and New Tab page]\(スタート ページと新しいタブ ページの事前起動\)**: **[ブロック]** を選択すると、スタート ページと新しいタブ ページが事前に起動されなくなります。 事前起動を行うと、Microsoft Edge のパフォーマンスが向上し、Microsoft Edge の起動時間が最小限に抑えられます。 **[Not configured]\(未構成\)** を選択すると、OS の既定の動作が使用され、これらのページが事前に起動される場合も、起動されない場合もあります。

### <a name="favorites-and-search"></a>お気に入りおよび検索

- **[お気に入りバー]**: Microsoft Edge のページ上のお気に入りバーに対する処理を選択します。 次のようなオプションがあります。
  - **[Not configured]\(未構成\)**: OS の既定の動作が使用され、すべてのページ上でお気に入りバーが非表示になる場合も、非表示にならない場合もあります。 この設定はユーザーが変更できます。
...
- **[Restrict changes to Favorites]\(お気に入りへの変更を制限する\)**: **[ブロック]** を選択すると、ユーザーはお気に入りの一覧の追加、インポート、並べ替え、編集ができなくなります。 **[Not configured]\(未構成\)** を選択すると、OS の既定値が使用され、ユーザーが一覧を変更できる場合も、変更できない場合もあります。
- **[Sync favorites between Microsoft browsers (desktop only)]\(Microsoft ブラウザー間でお気に入りを同期する (デスクトップのみ)\)**: **[必須]** を選択すると、Internet Explorer と Microsoft Edge 間でお気に入りが強制的に同期されます。 お気に入りに対する追加、削除、変更、および順序変更がブラウザー間で共有されます。  **[Not configured]\(未構成\)** を選択すると、OS の既定値が使用され、ユーザーがブラウザー間のお気に入りの同期を選択できる場合も、選択できない場合もあります。
- **[既定の検索エンジン]**: デバイス上の既定の検索エンジンを選択します。 エンド ユーザーはこの値をいつでも変更できます。 次のようなオプションがあります。
  ...
  - Yahoo
  - カスタム値
- **[検索候補]**: **[Not configured]\(未構成\)** を選択すると、アドレス バーに検索語句を入力したときに、検索エンジンからサイトが提案されるようになります。 **[ブロック]** を選択すると、この機能が停止されます。

### <a name="privacy-and-security"></a>プライバシーとセキュリティ

- **[InPrivate 閲覧]**: **[ブロック]** を選択すると、エンド ユーザーは InPrivate 閲覧セッションを開けなくなります。 **[Not configured]\(未構成\)** を選択すると、この機能が使用可能になります。
...
- **[パスワード マネージャー]**: **[ブロック]** を選択すると、Microsoft Edge の Password Manager 機能が無効になります。 **[Not configured]\(未構成\)** を選択すると、この機能が使用可能になります。
...
- **[トラッキング拒否ヘッダーを送信する]**: **[Not configured]\(未構成\)** を選択した場合、デバイスでは、トラッキング情報を要求する Web サイトにトラッキング拒否ヘッダーを送信する必要があります (推奨)。 **[ブロック]** を選択すると、デバイスからこれらのヘッダーが送信されなくなり、Web サイトでユーザーを追跡できるようになります。
...
- **[Full screen]\(全画面表示\)**: **[ブロック]** を選択すると、Microsoft Edge で Web コンテンツのみ表示されることが防止され、Microsoft Edge (全画面モード) が非表示になることが防止されます。 **[Not configured]\(未構成\)** を選択すると、OS の既定値が使用され、Microsoft Edge で全画面モードが使用されます。
- **[About flags]\(フラグについて\)**: **[ブロック]** を選択すると、開発者の設定および試験用の設定を含む Microsoft Edge 内の `about:flags` ページにエンド ユーザーがアクセスできなくなります。 **[Not configured]\(未構成\)** を選択すると、OS の既定値が使用され、`about:flags` ページへのアクセスが許可される場合も、許可されない場合もあります。
- **[開発者ツール]**: **[ブロック]** を選択すると、エンド ユーザーが Microsoft Edge 開発者ツールを開けなくなります。 **[Not configured]\(未構成\)** を選択すると、ユーザーが開発者ツールを開くことができます。
- **[拡張機能]**: **[Not configured]\(未構成\)** を選択すると、エンド ユーザーが Microsoft Edge 拡張機能をデバイス上にインストールできます。 **[ブロック]** を選択すると、インストールができなくなります。
...
- **[Display web results in search]\(検索で Web 結果を表示する\)**: デバイスで行われた検索での Web 結果の表示をブロックまたは許可します。
...
- **[Settings synchronization for Microsoft account]\(Microsoft アカウントの設定の同期\)**: Microsoft アカウントに関連付けられたデバイスとアプリの設定をデバイス間で同期できるようにします。
...
- **[VPN over the cellular network]\(移動体通信ネットワーク経由の VPN\)**: 移動体通信ネットワークに接続したときに、デバイスが VPN 接続にアクセスできるようにするかどうかを制御します。
- **[VPN roaming over the cellular network]\(移動体通信ネットワーク経由の VPN ローミング\)**: 移動体通信ネットワークでのローミング中に、デバイスが VPN 接続にアクセスできるようにするかどうかを制御します。
...
- **[[設定] アプリ]**: Windows の [設定] アプリへのアクセスをブロックします。
  - **[システム]**: [設定] アプリのシステム領域へのアクセスをブロックします。
    - **[Power and sleep settings modification (desktop only)]\(電源とスリープの設定の変更 (デスクトップのみ)\)**: エンド ユーザーがデバイスの電源とスリープの設定を変更することを防止します。
  ...
  - **[Time and Language]\(時刻と言語\)**: [設定] アプリの時刻と言語の領域へのアクセスをブロックします。
  ...
  - **[Ease of Access]\(簡単操作\)**: [設定] アプリの簡単操作の領域へのアクセスをブロックします。
  - **[プライバシー]**: [設定] アプリのプライバシー領域へのアクセスをブロックします。
  - **[Update and Security]\(更新とセキュリティ\)**: [設定] アプリの更新とセキュリティの領域へのアクセスをブロックします。
...
- **[Documents on Start]\([スタート] 上の [ドキュメント]\)**: Windows の [スタート] メニュー内にある [ドキュメント] フォルダーを表示または非表示にします。
- **[Downloads on Start]\([スタート] 上の [ダウンロード]\)**: Windows の [スタート] メニュー内にある [ダウンロード] フォルダーを表示または非表示にします。
- **[File Explorer on Start]\([スタート] 上の [エクスプローラー]\)**: Windows の [スタート] メニュー内にある [エクスプローラー] アプリを表示または非表示にします。
- **[HomeGroup on Start]\([スタート] 上の [ホームグループ]\)**: Windows の [スタート] メニュー内にある [ホームグループ] フォルダーを表示または非表示にします。
- **[Music on Start]\([スタート] 上の [ミュージック]\)**: Windows の [スタート] メニューにある [ミュージック] フォルダーを表示または非表示にします。
- **[Network on Start]\([スタート] 上の [ネットワーク]\)**: Windows の [スタート] メニュー内にある [ネットワーク] フォルダーを表示または非表示にします。
- **[Personal folder on Start]\([スタート] 上の [個人用フォルダー]\)**: Windows の [スタート] メニュー内にある個人用フォルダーを表示または非表示にします。
- **[Pictures on Start]\([スタート] 上の [ピクチャ]\)**: Windows の [スタート] メニュー内にあるピクチャ用のフォルダーを表示または非表示にします。
- **[Settings on Start]\([スタート] 上の [設定]\)**: Windows の [スタート] メニュー内にある [設定] アプリを表示または非表示にします。
- **[Videos on Start]\([スタート] 上の [ビデオ]\)**: Windows の [スタート] メニュー内にあるビデオ用のフォルダーを表示または非表示にします。
...
- **[キオスク モード]**: ポリシーによってサポートされるキオスク モードの種類を識別します。 次のオプションがあります。
  ...
  - **[Not configured]\(未構成\)** (既定): このポリシーでは、デバイス上でキオスク モードが有効になりません。
  ...
  - **[マルチ アプリ キオスク]**: このプロファイルでは、デバイスで多数のアプリを実行できます。 ユーザーはプロファイルに追加されているアプリだけを利用できます。 マルチ アプリ キオスク (または固定目的デバイス) の利点は、ユーザーがアクセスできるのは必要なアプリだけであり、必要のないアプリはビューから削除されているので、わかりやすいエクスペリエンスがユーザーに提供されることです。
...
- **[Use proxy script]\(プロキシ スクリプトを使う\)**: PAC スクリプトへのパスを入力してプロキシ サーバーを構成するには、このオプションを選択します。
  - **[セットアップ スクリプトのアドレスの URL]**: プロキシ サーバーの構成に使用する PAC スクリプトの URL を入力します。
- **[Use manual proxy server]\(手動プロキシ サーバーを使う\)**: プロキシ サーバーの情報を手動で入力するには、このオプションを選択します。
  - **[アドレス]**: プロキシ サーバーの名前または IP アドレスを入力します。
  - **[ポート番号]**: プロキシ サーバーのポート番号を入力します。
  - **[プロキシの例外]**: プロキシ サーバーを使用してはいけない URL を入力します。 各項目の区切りにはセミコロンを使用します。
  - **[Bypass proxy server for local address]\(ローカル アドレスにはプロキシ サーバーを使わない\)**: イントラネットでローカル アドレスのプロキシ サーバーを使用しない場合は、このオプションを有効にします。
...
- **[Windows スポットライト]**: この設定を使用し、Windows 10 デバイスですべての Windows スポットライト機能をブロックします。 この設定をブロックすると、次の設定は利用できません。
  - **[Windows Spotlight on lock screen]\(ロック画面上の Windows スポットライト\)**: デバイス ロック画面上で Windows スポットライトに情報が表示されません。
  - **[Third-party suggestions in Windows Spotlight]\(Windows スポットライトでのサード パーティのおすすめ\)**: Microsoft が発行していないコンテンツが Windows スポットライトで提案されません。
  - **[コンシューマー向けの機能]**: [スタート] メニューの提案、メンバーシップ通知などのコンシューマー機能をブロックできます。
  - **[Windows のヒント]**: Windows 内でのポップアップのヒントの表示をブロックできます。
  - **[Windows Spotlight in action center]\(アクション センターの Windows スポットライト\)**: 新しいアプリやセキュリティ コンテンツなどの Windows スポットライトの提案が Windows アクション センター内に表示されなくなります。
...
- **[User input from wireless display receivers]\(ワイヤレス ディスプレイ レシーバーからのユーザー入力\)**: ワイヤレス ディスプレイ レシーバーからのユーザー入力がブロックされます。
...
- **[MMS (mobile only)]\(MMS (モバイルのみ)\)**: デバイス上の MMS の送受信機能を無効にします。
