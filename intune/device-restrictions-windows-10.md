...
-   **[入力個人設定]** – Cortana、ディクテーション、Windows Store アプリのクラウドベースの音声入力サービスの使用を許可しません。 これらのサービスを許可する場合、Microsoft はサービス向上のために音声データを収集することがあります。
...
-   **[デバイスの Bluetooth 名]** - デバイスの Bluetooth 名を指定します。 名前を指定しない場合、既定のラジオ名が使用されます。
