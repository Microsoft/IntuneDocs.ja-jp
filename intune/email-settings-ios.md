...
- **[S/MIME]**: **[S/MIME を有効にする]** を選択すると、iOS ネイティブ メール アプリケーションで電子メールに対して署名または暗号化を行うことができます。 
  ...
  - **[S/MIME signing enabled]\(S/MIME 署名を有効にする\)**: **[有効にする]** を選択すると、管理者が入力したアカウントの送信電子メールにユーザーがデジタル署名できるようになります。 署名により、メッセージを受信するユーザーは、送信者になりすました他者ではなく、特定の送信者からメッセージが送信されたことを確認できます。 **[無効にする]** を選択すると、ユーザーがメッセージにデジタル署名することはできません。
    - **ユーザー設定を変更できる**: 選択**を有効にする**S/MIME 署名動作の変更を許可します。 **[無効にする]** を選択すると、管理者が構成した S/MIME 署名設定をユーザーが変更することはできません。 iOS 12 以降で使用できます。

  - **[S/MIME 署名証明書]**: 電子メール メッセージの署名に使用する既存の PKCS または SCEP 証明書プロファイルを選択します。
    - **ユーザー設定を変更できる**: 選択**を有効にする**署名証明書を変更できるようにします。 **[無効にする]** を選択すると、ユーザーが署名証明書を変更することはできず、管理者が構成した証明書を強制的に使用することになります。 iOS 12 以降で使用できます。

  - **既定で暗号化**:**を有効にする**は既定の動作としてすべてのメッセージを暗号化します。 **[無効にする]** を選択すると、既定の動作としてすべてのメッセージが暗号化されることはありません。
    - **ユーザー設定を変更できる**: 選択**を有効にする**既定の暗号化の動作を変更できるようにします。 **[無効にする]** を選択すると、ユーザーが既定の暗号化動作を変更することはできず、管理者が構成した設定を強制的に使用することになります。 iOS 12 以降で使用できます。
  ...
  - **[S/MIME 暗号化証明書]**: 電子メール メッセージの暗号化に使用する既存の PKCS または SCEP 証明書プロファイルを選択します。
    - **ユーザー設定を変更できる**: 選択**を有効にする**暗号化証明書を変更できるようにします。 **[無効にする]** を選択すると、ユーザーが暗号化証明書を変更することはできず、管理者が構成した証明書を強制的に使用することになります。 iOS 12 以降で使用できます。
