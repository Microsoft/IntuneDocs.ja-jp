...
| `WRONG_USER` | デバイスごとに 1 つのユーザーだけが、MAM サービスにアプリを登録できます。 この結果は、この結果が配信されたユーザー (2 番目のユーザー) が MAM ポリシーの対象となり、別のユーザーが既に登録されていることを示します。 2番目のユーザーには MAM ポリシーを適用できないため、後でこのユーザーの登録が成功するまで、またはこのユーザーのデータへのアクセスをアプリで許可しないようにする必要があります。 この `WRONG_USER` を提供すると同時に、MAM では、既存のアカウントを削除するオプションが表示されます。 人間のユーザーが肯定的な質問に答えた場合は、2番目のユーザーを少し後で登録することができます。 2番目のユーザーが登録されている限り、MAM は定期的に登録を再試行します。 |
| `UNENROLLMENT_SUCCEEDED` | 登録解除が正常に完了しました。|
| `UNENROLLMENT_FAILED` | 登録解除要求が失敗しました。  さらに詳細な情報がデバイス ログにあります。 一般に、アプリが有効な (null でも空でもない) UPN を渡す限り、この問題は発生しません。 アプリで実行できる信頼性の高い直接修復はありません。 有効な UPN の登録を解除するときにこの値を受け取った場合は、Intune MAM チームにバグとして報告してください。|
...
MAM に登録されたユーザーがいない場合でも、アプリで Android エンタープライズ構成 (特定のユーザーを対象としない) を取得する必要がある場合は、null または空の文字列を渡すことができます。

### <a name="conflicts"></a>競合
MAM アプリ構成で設定された値は、Android Enterprise config で設定されているキーと同じ値を上書きします。 

管理者が同じキーに対して競合する値を構成した場合 (たとえば、同じユーザーが属する複数のグループに対して同じキーを持つ異なるアプリ構成セットを対象にしている場合)、Intune にはこの競合を自動的に解決する方法がないため、すべての値が自動的に作成されます。アプリで使用できます。 
...
アプリ構成は、Graph API を使用して構成することもできます。 詳細については、MAM を対象とした[構成に関する Graph API のドキュメント](https://docs.microsoft.com/graph/api/resources/intune-mam-targetedmanagedappconfiguration)を参照してください。
...
[テストガイド](app-sdk-android-testing-guide.md)を参照してください。
