...
- **[Web サインイン]** : Windows サインインで、Security Assertion Markup Language (SAML) などの非 ADFS (Active Directory フェデレーション サービス) フェデレーション プロバイダーのサポートを有効にします。 SAML では、シングル サインオン (SSO) の操作性を提供するセキュリティで保護されたトークンを使用します。 次のようなオプションがあります。
