...
- ファースト**サインオン**: **[有効]** を選択すると、ユーザーが簡単に初めてサインインできるようになります。 **有効**にすると、デバイスは管理されていない新しい Azure AD アカウントを、事前に構成されたローカルアカウントに自動的に接続します。 クイック初回サインインエクスペリエンスを防ぐには、 **[無効]** を選択します。 **[未構成]** (既定値) の場合、この設定は Intune による管理対象外のままとなり、デバイスでこの設定を制御するためのポリシーはプッシュされません。
