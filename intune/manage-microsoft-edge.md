...
Intune Managed Browser と Microsoft Edge は両方とも、ポリシーで保護されているブラウザーとして使用できるようになりました。 ユーザーが確実に適切なブラウザー アプリを使用するように指示されるようにするには、以下の構成設定を使用して、すべての Intune マネージド アプリ (Outlook、OneDrive、SharePoint など) を対象にします。
