...
5. **[VPP トークンの作成]** ウィンドウで、次の情報を指定します。
    ...
    - **アプリの自動更新** - **[オン]** で **[オフ]** を選択し、自動更新を有効/無効にします。 有効にすると、アプリ ストア内の VPP アプリ更新プログラムが Intune によって検出され、デバイスのチェックイン時に自動的にデバイスにプッシュされます。 Apple VPP アプリの自動アプリ更新により、**必須**インストール インテントでデプロイされたアプリのみが自動的に更新されます。 **利用可能**インストール インテントでデプロイされたアプリでは、自動更新により、新しいバージョンのアプリが利用できることをお客様 (管理者) に知らせる通知が生成されます。 ユーザーは、新しいバージョンのアプリをクリックしてインストールする必要があります。 さらに、以前のバージョンのアプリがインストールされている場合でも、ユーザーに対して、ポータル サイトにアプリが未インストールとして表示されます。 その場合、ユーザーはアプリを再インストールできます。
