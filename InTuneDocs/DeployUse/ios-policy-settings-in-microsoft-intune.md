...
|**iCloud の写真共有を許可する**|iOS の共有フォト ストリームの機能の使用を許可します。|
|**iCloud フォト ライブラリを許可する**|iCloud に写真を格納することをユーザーに許可します。 無効にすると、iCloud に既に格納されている写真は削除されます。|
...
>
> -   iOS デバイスをキオスク モードに構成する前に、[Apple Configurator ツール](https://itunes.apple.com/us/app/apple-configurator/id434433123?mt=12)または [Apple Device Enrollment Program](ios-device-enrollment-program-in-microsoft-intune) を使用して、デバイスを監視下モードにする必要があります。 Apple Configurator ツールの詳細については、Apple のドキュメントを参照してください。
...
|**キーボード スペル チェックを許可する**|デバイスのスペル チェック機能を使用できるようにします。|
...
|**Apple Configuration と iTunes のみを使用してアプリのインストールを許可する**|デバイスのホーム画面から、アプリ ストアを有効または無効にします。 その場合でも、ユーザーは、iTunes を使用したり、Apple Configurator ツールを使用してアプリをインストールおよび更新できます。|
