...
1 iOS デバイスの場合、**[画面がオフになるまでの非アクティブな時間 (分)]** と **[デバイスの画面がロックされるまでの非アクティブな時間 (分)]** の設定を構成すると、順番に適用されます。 たとえば、両方の設定値を「 **5** 」分に設定すると、5 分後に画面が自動的にオフになり、さらに 5 分後にデバイスがロックされます。 ただし、ユーザーが手動で画面をオフにした場合、2 つ目の設定は即時に適用されます。 同じ例で、ユーザーが画面をオフにした後、デバイスは 5 分後にロックされます。
...
|設定の名前|Windows 8.1 および Windows RT 8.1|Windows RT|Windows Phone 8 および Windows Phone 8.1|iOS|Android および Samsung KNOX Standard|
|----------------|----------------------------------|--------------|-----------------------------------------|-------|----------------------------|
|**モバイル デバイスの暗号化を要求する**1<br /><br />Windows Phone 8 デバイスの場合、これを **[はい]**に設定する必要があります。<br /><br />iOS デバイスでの暗号化を有効にするには、 **[モバイル デバイスのロック解除にパスワードを必要とする]**設定を有効にします。|○|×|○|×|○|
|**メモリ カードの暗号化を必要とする**<br /><br />この設定は、Exchange ActiveSync によって管理されるデバイスにも当てはまります。|該当なし|該当なし|該当なし <br />アプリおよび関連付けられたデータは自動的に暗号化されます。|該当なし|○|
1 Windows 8.1 が実行されているデバイスの追加情報:
...
|**工場出荷時のリセットを許可する**|×|×|×|×|○ (Samsung KNOX Standard のみ)|
...
|**暗号化されたバックアップを必須にする**|×|×|×|○|×|
|**作業フォルダーの URL**<br /><br />この設定は、作業フォルダーの URL を設定して、デバイス間でドキュメントを同期できるようにします。|○|×|×|×|×|
|**Google へのバックアップの許可**|×|×|×|×|○ (Samsung KNOX Standard のみ)|
...
|**Google アカウントの自動同期を許可する**|×|×|×|×|○ (Samsung KNOX Standard のみ)|

## <a name="email-settings"></a>電子メールの設定

|設定の名前|Windows 8.1 および Windows RT 8.1|Windows RT|Windows Phone 8 および Windows Phone 8.1|iOS|Android および Samsung KNOX Standard|
|----------------|----------------------------------|--------------|-----------------------------------------|-------|----------------------------|
|**ユーザーが電子メールの添付ファイルをダウンロードできるようにする**1|該当なし|該当なし|該当なし|該当なし|該当なし|
...
|**オートコンプリートを使用する**|○|×|×|○|○ (Samsung KNOX Standard のみ)|
|**ポップアップ ブロックを使用する**|○|×|×|○|○ (Samsung KNOX Standard のみ)|
|**Cookie を使用する**|×|×|×|○|○ (Samsung KNOX Standard のみ)|
|**プラグインを使用する**|○|×|×|×|×|
|**アクティブ スクリプティングを使用する**|○|×|×|○|○ (Samsung KNOX Standard のみ)|
|**不正行為の警告を使用する**|○|×|×|○|×|
|**1 単語の入力でイントラネット サイトにアクセスできるようにする**<br /><br />(この設定を使うと、"Bing" のような 1 つの単語を使用すると Internet Explorer が Web サイトにアクセスするようになります。)|○|×|×|×|×|
...
|**リムーバブル記憶域を許可する**|×|×|○|×|○ (Samsung KNOX Standard のみ)|
...
|**電源の切断を許可する**<br>この設定が無効の場合、Samsung KNOX Standard デバイスの **[デバイスをワイプするまでの連続サインイン エラーの回数]** 設定は機能しません。|×|×|×|×|○ (Samsung KNOX Standard のみ)|

## <a name="device-capabilities-settings---cellular"></a>デバイス機能の設定 - 移動体通信

|設定の名前|Windows 8.1 および Windows RT 8.1|Windows RT|Windows Phone 8 および Windows Phone 8.1|iOS|Android および Samsung KNOX Standard|
|----------------|----------------------------------|--------------|-----------------------------------------|-------|----------------------------|
|**音声通話ローミングを許可する**|×|×|×|○|○ (Samsung KNOX Standard のみ)|
|**データ ローミングを許可する**|○|×|×|○|○ (Samsung KNOX Standard のみ)|
|**ローミング中の自動同期を許可する**|×|×|×|○|×|
|**SMS/MMS メッセージングを許可する**|×|×|×|×|○ (Samsung KNOX Standard のみ)|

## <a name="device-capabilities-settings---features"></a>デバイス機能の設定 - 機能

|設定の名前|Windows 8.1 および Windows RT 8.1|Windows RT|Windows Phone 8 および Windows Phone 8.1|iOS|Android および Samsung KNOX Standard|
|----------------|----------------------------------|--------------|-----------------------------------------|-------|----------------------------|
|**音声アシスタントを使用する**|×|×|×|○|○ (Samsung KNOX Standard のみ)|
|**デバイスのロック中に音声アシスタントを使用する**|×|×|×|○|×|
|**音声による発信を使用する**|×|×|×|○|○ (Samsung KNOX Standard のみ)|
|**コピーと貼り付けを許可する**|×|×|Windows Phone 8.1 のみ|×|○ (Samsung KNOX Standard のみ)|
|**アプリケーション間でのクリップボードの共有を許可する**|×|×|×|×|○ (Samsung KNOX Standard のみ)|
|**YouTube を許可する**|×|×|×|×|○ (Samsung KNOX Standard のみ)|
