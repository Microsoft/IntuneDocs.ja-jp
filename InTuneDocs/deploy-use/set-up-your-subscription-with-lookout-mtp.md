...
Lookout デバイス脅威保護を設定するには、次の手順を実行します。
...
Lookout を Intune に統合するために、Lookout Mobility Endpoint Security テナントが Azure AD サブスクリプションに関連付けられます。 Lookout デバイス脅威保護サービスのサブスクリプションを有効にするには、Lookout のサポート (enterprisesupport@lookout.com) に次の情報を伝えてください。  
...
4. 以上の情報を収集した後、Lookout のサポートに依頼します (電子メール: enterprisesupport@lookout.com)。 Lookout のサポート担当者は、お客様の連絡窓口となっている方と協力し、収集された情報を使用してサブスクリプションの登録と Lookout Enterprise アカウントの作成を行います。
