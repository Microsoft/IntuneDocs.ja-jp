...
## <a name="your-device-is-missing-a-certificate-that-usually-comes-installed-on-your-phone"></a>通常は電話にインストールされている証明書がデバイスにない
...
4.  **[証明書の名前指定]** ダイアログ ボックスで、既定の証明書名を受け入れます。
...
## <a name="your-device-is-missing-a-certificate-required-by-your-it-admin"></a>デバイスには、IT 管理者が必要とする証明書がありません
Android デバイスが Intune に登録されておらず、IT 管理者が必要とする特定の証明書がない場合は、Android ポータル サイト アプリにサインインすることはできません。 サインインしようとすると、次のメッセージが表示されます。

![screenshot-error-message-about-missing-certificate](./media/andr-cert_install-1-cert_missing.png)

>[!NOTE]
> 既に "証明書が見つかりません" というメッセージが 1 つ表示され、[「通常は電話にインストールされている証明書がデバイスにない」](#your-device-is-missing-a-certificate-that-usually-comes-installed-on-your-phone)に記載の手順に従っている場合は問題ありません。 それはこのメッセージと証明書とは別のものです。そのため、欠落している証明書を取得するには、このセクションの手順に従ってください。
...
上のような "証明書が見つかりません" というメッセージが表示されたが、この手順を既に実行している場合、別の証明書が存在する可能性があります。IT 管理者にそのインストールを依頼する必要があります。 IT 管理者に連絡し、[Android 証明書の問題](/intune/troubleshoot/troubleshoot-device-enrollment-in-intune#android-certificate-issues)のリンクを与えます。問題の解決に役立つ手順が記載されています。
