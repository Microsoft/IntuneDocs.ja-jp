...
| パスワードには複雑な文字を含める必要があります。 | 数字、大文字、記号などの複雑な文字を、 `$`、 `%`などの`#`ように追加します。 組織では、他のユーザーがパスワードを推測しづらいように、文字、数字、英数字以外の文字を組み合わせて使用する必要があります。|  
...
| パスコードが最近使用されました。| 以前に使用していないパスコードを選択してください。 組織では、パスコードを再利用する前に一定の時間が経過している必要があります。 |
|タッチ ID または顔 ID 認証が必要です。 | タッチ ID または顔 ID を設定します。 パスワードまたはクレジットカード情報にオートコンプリートを使用する前に、これらの方法のいずれかを使用して認証を行う必要があります。 | 

## <a name="macos-password-requirements"></a>macOS パスワードの要件
| メッセージ | 修正方法 |
|-----------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| パスワードが必要です。 | パスワードを設定してください。 組織では、デバイスのロックを解除するためにパスワードを入力する必要があります。 |
| パスワードが簡単すぎます。|  パスワードには、1234や1111のように、連続する数値や繰り返しの番号が含まれていないことを確認してください。 |
...
| パスワードには、英数字と記号を含める必要があります。 | 文字、数字、および特殊文字`&` `$`(、 `!` `%` `#`、、、など) の組み合わせを含むパスワードを設定します。 |
